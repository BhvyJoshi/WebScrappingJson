...
public class SouthEssexHelperClass {
    ...
    private String[] modifyHeader(String[] hdr)
    {
        ...
        header = header.replace("DATE","DATE__c");
        header = header.replace("Type","Type__c").replace("FIRST PARTY NAME","FIRST_PARTY_NAME__c").replace("ROLE","ROLE__c");
        header = header.replace("Book","Book__c").replace("Page","Page__c").replace("Type","Type__c").replace("SECOND PARTY NAME","SECOND_PARTY_NAME__c");
        header = header.replace("DESC","DESC__c").replace("Street","Street__c").replace("Town","Town__c");
        header = header.replace("Locus","Locus__c").replace("PBK","PBK__c").replace("PPG","PPG__c").replace("Consideration","Consideration__c");
